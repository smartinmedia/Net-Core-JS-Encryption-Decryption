...
{
    public static class EncryptionHandler2
    {
        public static string DecryptStringAES()
        {
            var keybytes = EncryptionHandler.GenerateXBytesOfRandomEntropy(16);//Encoding.UTF8.GetBytes("7061737323313233");
            var iv = Encoding.UTF8.GetBytes("7061737323313233");

            //c# encrrption
            var encryptStringToBytes = EncryptStringToBytes("It works", keybytes, iv);
            var encBase64 = Convert.ToBase64String(encryptStringToBytes);
            ...
            return string.Format(
                "roundtrip reuslt:{0}{1}Javascript result:{2} and result base64: {3}",
                roundtrip,
                Environment.NewLine,
                decriptedFromJavascript, encBase64);
        }
