...
The encryption / decryption uses the WebCrypto (Crypto.subtle) supported by current browsers, is super-fast, asynchronous and pretty straight forward. It relies on JS promises. The Scrypt functionality can be used synchronous or asynchronous, which is a little bit more tricky. For **asynchronous, you have to provide
...
<br/>The following Javascript is from the "index.html". Try it out!
